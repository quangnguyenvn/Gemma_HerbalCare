...
- **Edge server / clinic / NGO laptop:** this is the local NGO option. A small team in one region, such as Bolivia, Sudan, Pakistan, India, northern Vietnam, or another rural area, could download the project, host it on a clinic laptop or small server, and use an efficient Gemma 4 MoE/A4B-class endpoint for local people without needing global-scale infrastructure.
...
- **Behavior fine-tuning for safer guidance:** future fine-tuning should not teach Gemma to diagnose disease or invent herbal knowledge. It should tune response behavior only: safer refusal style, emergency escalation wording, low-literacy explanations, multilingual phrasing, and structured outputs such as `ESCALATE`, `REFUSE`, `ASK_MORE`, and `EXPLAIN_CURATED_RECORDS`. The rule-based safety gate remains the authority; fine-tuning must not replace deterministic triage. Gemma should learn how to communicate safely, not decide medical truth. Evaluation should include refusal tests, emergency recall tests, hallucination tests, and low-literacy readability checks.
